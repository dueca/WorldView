...
// constructor
WorldView::WorldView ( Entity* e, const char* part, const
                       PrioritySpec& ps ) :
...
bool WorldView::setWindowEventWriting ( const vector<std::string>& s )
{
  ...
  if ( s.size() == 1 ) {
    efb.w_event.reset(new ChannelWriteToken
      (getId(), NameSet(getEntity(), "WorldViewerEvent", ""),
    ...
    for (auto &wchannel: m_others) {
      while (wchannel->checkChange(ei)) {
        if (ei.created) {
          ...
          // remove an entry
          rendersurface->removeControllable
            (wchannel->getChannelName(), ei.creation_id);
        }
      }
    }
