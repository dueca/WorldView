...
public:
  ...
  struct GLContextSet {
    ...
    ::GLXContext context;
    ::Window     xwin;
    GLContextSet(::Display *dpy = NULL, 
		 ::GLXContext context = NULL, ::Window xwin = 0) :
      dpy(dpy), context(context), xwin(xwin) { }
  };
private:
  ...
  struct ViewSet {
    ...
    void init(const ViewSpec& vs, WindowSet& window, 
	      Ogre::SceneManager *scene, int zorder); 
  ...
  struct WindowSet {
    ...
    WindowSet() : 
#ifdef OWN_WIN_AND_CONTEXT
      dpy(NULL), glc(NULL), xwin(0), 
#endif
      switcher(NULL), window(NULL) { }
...
protected:
  ...
  struct ResourceSpec {
    ...
    ResourceSpec(const std::string& l, const std::string& t,
      ...
      location(l), type(t), group(g), recursive(r) { }
  };
...
private:
  ...
  typedef std::map<creation_key_t,
                   boost::intrusive_ptr<OgreObject> > created_objects_t;
