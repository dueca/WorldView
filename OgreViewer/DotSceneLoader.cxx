...
void DotSceneLoader::processLight(TiXmlElement *XMLNode, SceneNode *pParent)
{
	...
	// limit the distance over which shadows are draw to limit CPU time
	pLight->setShadowFarDistance(500);
	TiXmlElement *pElement;
...
{
	// get the attributes of the BillBoardSet
	String name = getAttrib(XMLNode, "name");
	String material = getAttrib(XMLNode, "material");
	String id = getAttrib(XMLNode, "id");
	Real width = getAttribReal(XMLNode, "width", 10);
	Real height = getAttribReal(XMLNode, "height", 10);	
	String type = getAttrib(XMLNode, "type", "point");
	String origin = getAttrib(XMLNode, "origin", "center");
	
	LogManager::getSingleton().logMessage("[DotSceneLoader] Creating a BillBoardSet!");
	
	BillboardSet* bSet = 0;
	// try to create the BillBoardSet
	try
	{
		bSet = mSceneMgr->createBillboardSet(name);
		// set the origin of the BillBoards in the Set
		Ogre::BillboardOrigin bOrigin = Ogre::BBO_CENTER;
		if(origin == "bottomLeft")
			bOrigin = Ogre::BBO_BOTTOM_LEFT;
		else if(origin == "bottomRight")
			bOrigin = Ogre::BBO_BOTTOM_RIGHT;
		else if(origin == "left")
			bOrigin = Ogre::BBO_CENTER_LEFT;
		else if(origin == "right")
			bOrigin = Ogre::BBO_CENTER_RIGHT;
		else if(origin == "topLeft")
			bOrigin = Ogre::BBO_TOP_LEFT;
		else if(origin == "topCenter")
			bOrigin = Ogre::BBO_TOP_CENTER;
		else if(origin == "topRight")
			bOrigin = Ogre::BBO_TOP_RIGHT;
		else
			bOrigin = Ogre::BBO_CENTER;
		
		bSet->setBillboardOrigin(bOrigin);

		
		Ogre::BillboardType bType = Ogre::BBT_POINT;
		// set the type of BillBoards in the Set
		//orientedCommon | orientedSelf | point
		if(type == "orientedCommon")
			bType = Ogre::BBT_ORIENTED_COMMON;
		else if(type == "orientedSelf")
			bType = Ogre::BBT_ORIENTED_SELF;
		else 
			bType = Ogre::BBT_POINT;
		
		bSet->setBillboardType(bType) ;

		bSet->setDefaultDimensions(width, height);
		bSet->setCastShadows(true);
		
		if(!material.empty())
			bSet->setMaterialName(material);
	
		pParent->attachObject(bSet);
		
	}
	catch(Ogre::Exception &/*e*/)
	{
		LogManager::getSingleton().logMessage("[DotSceneLoader] Error creating a BillBoardSet!");
	}
	
	// the billboardset has been defined. Now process all the billboards within the billboardset
	TiXmlElement *pElement = XMLNode->FirstChildElement("billboard");
	while(pElement)
	{
		processBillboard(pElement, bSet);
		pElement = pElement->NextSiblingElement("billboard");
	}
	
}

void DotSceneLoader::processBillboard(TiXmlElement *XMLNode, BillboardSet *pParent)
{
	String id = getAttrib(XMLNode, "id");
	Real width = getAttribReal(XMLNode, "width", 10);
	Real height = getAttribReal(XMLNode, "height", 10);
	...
	pElement = XMLNode->FirstChildElement("colorDiffuse");
	ColourValue colour(0.0, 0.0, 0.0, 1.0);
	if(pElement)
	{
		colour = parseColour(pElement);
	}
	...
	try
	{
		bBoard = pParent->createBillboard(position);
		bBoard->setDimensions(width, height);
		bBoard->setColour(colour);
	}
	catch(Ogre::Exception &/*e*/)
	{
		LogManager::getSingleton().logMessage("[DotSceneLoader] Error creating a BillBoard!");
	}
}


/** This creates a plane. Handy for creating groundplanes
 	@author Mark Mulder (mark.mulder@tudelft.nl)
 	@param XMLNode the current xmlnode from the ogrescene we're loading
 	@param pParent a scenenode where we can attach the plane to so the scene manager can display it
 */
void DotSceneLoader::processPlane(TiXmlElement *XMLNode, SceneNode *pParent)
{	
	LogManager::getSingleton().logMessage("[DotSceneLoader] Creating ground plane");
	String name = getAttrib(XMLNode, "name");
	String id = getAttrib(XMLNode, "id");
	Real distance = getAttribReal(XMLNode, "distance", 0);	
	...
	// Normal pointing up
	Ogre::Plane plane(Ogre::Vector3::UNIT_Y, distance);
	try
	{
		// Create the plane
		Ogre::MeshManager::getSingleton().createPlane(name, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME,
										...
										uTile, vTile, Ogre::Vector3::UNIT_Z);
		// Create the entity
		Entity *pEntity = 0;
		// Create the mesh
		pEntity = mSceneMgr->createEntity(id, name);
		// make sure it doesn't cast any shadows
...
String DotSceneLoader::getAttrib(TiXmlElement *XMLNode, const String &attrib, const String &defaultValue)
{
	if(XMLNode->Attribute(attrib))
	        return String(*(XMLNode->Attribute(attrib)));
	else
		return defaultValue;
}
...
void DotSceneLoader::processUserDataReference(TiXmlElement *XMLNode, Entity *pEntity)
{
	...
	pEntity->getUserObjectBindings().setUserAny(Any(str));
#else
