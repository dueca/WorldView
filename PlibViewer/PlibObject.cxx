...
PlibObject::PlibObject(const WorldDataSpec& spec) :
  ...
  model(NULL)
{
  if (spec.filename.size()) { modelfile = spec.filename[0]; }
  this->name = spec.name;
  for (double& s: scale) { s = 1.0; }
}
...
void PlibObject::iterate(TimeTickType ts,
			 ...
			 double late)
{
  if (r_motion->isValid() && to_model != NULL) {
	...
	sgCoord position = AxisTransform::plibCoordinates
	  (o2.xyz, o2.attitude_q);
	to_model->setTransform(&position, scale[1], scale[0], scale[2]);
      }
      else {
	sgCoord position = AxisTransform::plibCoordinates
	  (r.data().xyz, r.data().attitude_q);
	to_model->setTransform(&position, scale[1], scale[0], scale[2]);
      }        
    }
...
static SubContractor<PlibObjectTypeKey, PlibObject>
*PlibObjectMoving_maker = new
    SubContractor<PlibObjectTypeKey, PlibObject>
    ("PlibObjectMoving");
