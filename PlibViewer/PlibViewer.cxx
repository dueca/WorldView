...
/** Name of this object */
  ...
  std::list<PlibViewer::ViewSet> viewsets;
    ...
    /** Constructor */
  PlibViewer_WindowSet(const std::string &name = "",
                       PlibViewer *viewer = NULL) :
    name(name),
    viewer(viewer),
    dpy(NULL),
    glc(NULL),
...
inline PlibViewer_WindowSet PlibViewer::myCreateWindow(const WinSpec &ws)
{
  ...
  swa.event_mask = ExposureMask | StructureNotifyMask
    ...
    ;
  res.xwin = XCreateWindow(res.dpy, xroot, 0, 0, ws.size_and_position[0],
                           ws.size_and_position[1], 0, vi->depth, InputOutput,
  ...
  if (imp->shared_context == NULL)
    imp->shared_context = res.glc;

  // remove mouse pointer
  if (!keep_pointer) {
    static const char emptycursor_bits[6] = { 0x00 };
    Pixmap cursor_pixmap =
      XCreateBitmapFromData(res.dpy, res.xwin, emptycursor_bits, 1, 1);
    XColor black;
...
{
  glClearColor(viewer->bg_color[0], viewer->bg_color[1], viewer->bg_color[2],
               0);
  ...
  for (list<PlibViewer::ViewSet>::const_iterator ii = viewsets.begin();
    ...
    //    viewer->orientation);
    double camquat[4];
    QxQ(camquat, viewer->orientation, ii->angle_offset);
...
void PlibViewer::init(bool waitswap)
{
  ...
  while (!viewspec.empty()) {
    ...
    if (ii == windows.end()) {
      ...
      ii->second.viewsets.back().init(viewspec.front(), ii->second, scene,
                                      zorder++);
      viewspec.pop_front();
    }
  }
  ...
  for (auto &ob : active_objects) {
    ob.second->init(scene);
  }
  for (auto &ob : static_objects) {
    ob->init(scene);
  }
...
void PlibViewer::ViewSet::init(const ViewSpec &spec,
                               const PlibViewer_WindowSet &ws, ssgRoot *scene,
                               int zorder)
{
  cout << "Creating camera \"" << spec.name << "\" in window \"" << ws.name
       << '"' << endl;
...
bool PlibViewer::createControllable(const GlobalId &master_id,
...
void PlibViewer::redraw(bool wait, bool reset_context)
{
  ...
  for (WindowsMap::iterator ii = windows.begin(); ii != windows.end(); ii++) {
...
void PlibViewer::setBase(TimeTickType tick, const BaseObjectMotion &base,
...
{
  WorldDataSpec obj =
    retrieveFactorySpec(name[0], "", static_objects.size(), true);
  if (obj.type.size() == 0) {
...
bool PlibViewer::setEventMask(const std::string &window, unsigned long mask)
{
  ...
  for (std::list<WinSpec>::iterator jj = winspec.begin(); jj != winspec.end();
       jj++) {
    if (jj->name == window) {
...
const WorldViewerEvent &PlibViewer::getNextEvent(const std::string &window)
{
  ...
  XEvent x_event;
  Bool res = XCheckWindowEvent(ii->second.dpy, ii->second.xwin, keysandpointer,
                               &x_event);
  if (!res) {
    return current_event;
  }
  ...
  case KeyPress:
  case KeyRelease:
    current_event.type = (x_event.type == KeyPress)
                           ? WorldViewerEvent::WVKeyPress
    ...
    current_event.code =
      XkbKeycodeToKeysym(ii->second.dpy, x_event.xkey.keycode, 0, 0);
    current_event.state = x_event.xkey.state;
    break;
  case ButtonPress:
  case ButtonRelease:
    current_event.type = (x_event.type == ButtonPress)
                           ? WorldViewerEvent::WVButtonPress
