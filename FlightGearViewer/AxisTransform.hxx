...
// a matrix that takes external storage
typedef Eigen::Map<Eigen::Matrix<float,Eigen::Dynamic,Eigen::Dynamic,Eigen::RowMajor> > MatrixfE;
// a const matrix that takes external storage
typedef Eigen::Map<const Eigen::Matrix<float,Eigen::Dynamic,Eigen::Dynamic,Eigen::RowMajor> > cMatrixfE;
// a const matrix that takes external storage
...
struct FGAxis
{
  ...
  virtual void transform(double result[6], 
			 const double xyz[3], const double quat[4]) = 0;
  ...
  virtual void toECEF(double pos[3],
		      float velocity[3],
		      float attitude[4],
		      float omega[3],
		      const double xyz[3],
...
struct FGLocalAxis: public FGAxis
{
  ...
  FGLocalAxis(double lat_zero, double lon_zero, double h_zero,
	      double psi_zero);
  ...
  /** Result of this transformation is latitude, in degrees, 
      ...
      \param  xyz    position vector
      \param  quat   attitude quaternion. */
  void transform(double result[6], 
		 const double xyz[3], const double quat[4]);

  /** Result of this transformation is ECEF position, double, in m
      Standard ECEF, x through Greenwich meridian, y through east, z upwards */
  void toECEF(double pos[3],
	      float velocity[3],
	      float attitude[4],
	      float omega[3],
	      const double xyz[3],
