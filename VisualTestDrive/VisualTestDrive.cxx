...
const ParameterTable* VisualTestDrive::getMyParameterTable()
{
  static const ParameterTable parameter_table[] = {
    { "set-timing", 
      new MemberCall<VisualTestDrive,TimeSpec>
        (&VisualTestDrive::setTimeSpec), set_timing_description },

    { "check-timing", 
      new MemberCall<VisualTestDrive,vector<int> >
      (&VisualTestDrive::checkTiming), check_timing_description },
    ...
    { "dt", 
      new MemberCall<VisualTestDrive,double>(&VisualTestDrive::setDt), 
    ...
    { NULL, NULL, 
      "Test data for view module"} };

  return parameter_table;
}

// constructor
VisualTestDrive::VisualTestDrive(Entity* e, const char* part, const
		       PrioritySpec& ps) :
...
{ 
  if (temp.name.size()) {
    if (!ego.initial.name.size()) {
      ego = MotionSet(temp, NULL);
    }
    else {
      motion_sets.push_back
	(MotionSet(temp, 
		   new MultiStreamWriteToken<ObjectMotion>
		   (getId(), NameSet("world", "ObjectMotion", ""), 2000)));
    }
  }
...
void VisualTestDrive::fillSnapshot(const TimeSpec& ts, 
...
void VisualTestDrive::doCalculation(const TimeSpec& ts)
{
  ...
  StreamWriter<ObjectMotion> w(w_ego, ts);
  w.data() = ego.moving;

  for (list<MotionSet>::iterator ii = motion_sets.begin();
       ii != motion_sets.end(); ii++) {
    MultiStreamWriter<ObjectMotion> w(*(ii->w_entity), ts);
    w.data() = ii->moving;
  }
